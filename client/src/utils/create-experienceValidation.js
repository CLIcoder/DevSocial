export const createExperienceValidation = ({
  ...
  if (location.length < 1 || location.length > 15)
  ...
  if (to.length < 1) error.to = "it should not be empty ";
  if (from.length < 1) error.from = "it should not be empty ";
...
export const createEducationValidation = ({
  ...
  if (school.length < 1 || school.length > 15)
    error.school = "please provide a valide school name";
  if (description.length < 10 || description.length > 100)
    error.description = "At least 10 caracters";
  if (fieldofstudy.length < 1 || fieldofstudy.length > 15)
    error.fieldofstudy = "please provide a valide fieldofstudy";
  if (degree.length < 1 || degree.length > 15)
